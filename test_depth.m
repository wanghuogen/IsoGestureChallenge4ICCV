clear,clc
parent_dir = 'image4tensorflow/test/';
Norm4S_dir1 = [parent_dir,'test_depth_fullbody/'];
if ~exist(Norm4S_dir1)
    mkdir(Norm4S_dir1);
end
Norm4S_dir2 = [parent_dir,'test_depth_full/'];
...
ff_train1 = fopen('image4tensorflow/test/test_depth_fullbody.txt','w');
ff_train2 = fopen('image4tensorflow/test/test_depth_full.txt','w');

%ff_val = fopen('val.txt','w');
while feof(fpn) ~= 1                %用于判断文件指针p在其所指的文件中的位置，如果到文件末，函数返回1，否则返回0  
      file = fgetl(fpn);           %获取文档第一行  
      obj_origin = VideoReader(['DATA/',file(22:41)]);
      detectionLabel = fopen(['DATA/DetectionLabel4Depth4test/',file(27:29),'/Label_',file(31:37),'.txt']);
      disp(['Extract frame of ',file(31:37)]);
      ...
      outImageNamef = fullfile(Norm4S_dir1,file(27:29),file(31:37));
      outImageNamer = fullfile(Norm4S_dir2,file(27:29),file(31:37));
      if ~exist(fullfile(Norm4S_dir1,file(27:29),file(31:37)))
          mkdir(fullfile(Norm4S_dir1,file(27:29),file(31:37)));
      end
      if ~exist(fullfile(Norm4S_dir2,file(27:29),file(31:37)))
          mkdir(fullfile(Norm4S_dir2,file(27:29),file(31:37)));
      end
