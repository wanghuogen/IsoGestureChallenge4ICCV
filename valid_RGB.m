...
Norm4S_dir1 = [parent_dir,'valid_Rgb_fullbody/'];
if ~exist(Norm4S_dir1)
    mkdir(Norm4S_dir1);
end
Norm4S_dir2 = [parent_dir,'valid_Rgb_full/'];
...
ff_train1 = fopen('image4tensorflow/valid/valid_rgb_fullbody.txt','w');
ff_train2 = fopen('image4tensorflow/valid/valid_rgb_full.txt','w');
...
while feof(fpn) ~= 1                %用于判断文件指针p在其所指的文件中的位置，如果到文件末，函数返回1，否则返回0  
      ...
      detectionLabel = fopen(['DATA/DetectionLabel4Rgb4valid/',file(7:9),'/Label_',file(11:17),'.txt']);
