...
testing_datalist = 'image4tensorflow/test/test_depth_full.txt'
...
load_params = tl.files.load_npz(name='depth_full_model.npz')
...
for X_indices, y_label_t in tl.iterate.minibatches(X_teidx,
  ...
  print predict_value
  np.save('test_depth_full_predict/' + image_path[0][-7:] + '.npy', predict_value)
