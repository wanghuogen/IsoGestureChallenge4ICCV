import os
os.environ['CUDA_VISIBLE_DEVICES'] = '0' #change based your machine
import io
import sys
import numpy as np
...
num_classes = 249
dataset_name = 'depth_full'
training_datalist = 'image4tensorflow/train/train_depth_full.txt'
testing_datalist = 'image4tensorflow/train/val_depth_full.txt'
...
for epoch in range(n_epoch):
  ...
  for X_indices, y_label_t in tl.iterate.minibatches(X_teidx, 
    ...
    image_info = zip(image_path,image_fcnt,image_olen,is_training)
    X_data_t = tl.prepro.threading_data([_ for _ in image_info], data.prepare_isogr_depth_data)
