...
Norm4S_dir1 = [parent_dir,'test_Rgb_fullbody/'];
if ~exist(Norm4S_dir1)
    mkdir(Norm4S_dir1);
end
Norm4S_dir2 = [parent_dir,'test_Rgb_full/'];
...
ff_train1 = fopen('image4tensorflow/test/test_rgb_fullbody.txt','w');
ff_train2 = fopen('image4tensorflow/test/test_rgb_full.txt','w');

while feof(fpn) ~= 1                %用于判断文件指针p在其所指的文件中的位置，如果到文件末，函数返回1，否则返回0  
      file = fgetl(fpn);           %获取文档第一行  
      obj_origin = VideoReader(['DATA/',file(1:20)]);
      detectionLabel = fopen(['DATA/DetectionLabel4Rgb4test/',file(6:8),'/Label_',file(10:16),'.txt']);
      disp(['Extract frame of ',file(10:16)]);
      ...
      outImageNamef = fullfile(Norm4S_dir1,file(6:8),file(10:16));
      outImageNamer = fullfile(Norm4S_dir2,file(6:8),file(10:16));
      if ~exist(fullfile(Norm4S_dir1,file(6:8),file(10:16)))
          mkdir(fullfile(Norm4S_dir1,file(6:8),file(10:16)));
      end
      if ~exist(fullfile(Norm4S_dir2,file(6:8),file(10:16)))
          mkdir(fullfile(Norm4S_dir2,file(6:8),file(10:16)));
      end
