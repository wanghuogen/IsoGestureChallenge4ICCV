...
Norm4S_dir1 = [parent_dir,'test_DepthDIf_full/'];
if ~exist(Norm4S_dir1)
    mkdir(Norm4S_dir1);
end
Norm4S_dir2 = [parent_dir,'test_DepthDIr_full/'];
...
while feof(fpn) ~= 1                %用于判断文件指针p在其所指的文件中的位置，如果到文件末，函数返回1，否则返回0  
      ...
      disp(['Processing DIs of ',file(31:37)]);
      ...
      outImageNamef = fullfile(Norm4S_dir1,file(27:29),sprintf('%s.jpg',file(31:37)));
      outImageNamer = fullfile(Norm4S_dir2,file(27:29),sprintf('%s.jpg',file(31:37)));
      
      if ~exist(fullfile(Norm4S_dir1,file(27:29)))
          mkdir(fullfile(Norm4S_dir1,file(27:29)));
      end
      if ~exist(fullfile(Norm4S_dir2,file(27:29)))
          mkdir(fullfile(Norm4S_dir2,file(27:29)));
      end
